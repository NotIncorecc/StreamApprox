...
def main():
    # 1. Setup paths
    features_path = "features/train_features.pt"
    if not os.path.exists(features_path):
        features_path = "data/train_features.pt"
        
    output_dir = "features/analysis_plots_train_features"
    os.makedirs(output_dir, exist_ok=True)
    
    print("=" * 60)
    print("ANALYZING MULTIMODAL TRAIN FEATURES")
    ...
    non_zero_aud_mean = 0.0
    non_zero_aud_std = 0.0
    if len(non_zero_aud) > 0:
        non_zero_aud_mean = non_zero_aud.mean().item()
        non_zero_aud_std = non_zero_aud.std().item()
        print(f"  z_aud L2 (non-zero): {non_zero_aud_mean:.4f} +/- {non_zero_aud_std:.4f}")
    ...
    # Use a subset for pairwise matrix to prevent Out-Of-Memory/Slow operations on 6.6k samples
    subset_size_sim = min(1000, num_samples)
    sim_img_img = torch.mm(z_img_norm[:subset_size_sim], z_img_norm[:subset_size_sim].t())
    sim_teacher_teacher = torch.mm(v_teacher_norm[:subset_size_sim], v_teacher_norm[:subset_size_sim].t())
    
    off_diag_mask = ~torch.eye(subset_size_sim, dtype=torch.bool)
    off_diag_sim_img = sim_img_img[off_diag_mask]
    off_diag_sim_teacher = sim_teacher_teacher[off_diag_mask]
    
    print(f"Within-Modality Self-Similarity (Pairwise Cosine Similarity between first {subset_size_sim} samples):")
    ...
    # Check audio similarity for non-zero vectors
    valid_aud_idx = (norm_aud > 1e-5).nonzero().squeeze(1)
    valid_aud_sim_mean, valid_aud_sim_std = 0.0, 0.0
    valid_aud_sim_min, valid_aud_sim_max = 0.0, 0.0
    if len(valid_aud_idx) > 1:
        aud_subset_size = min(1000, len(valid_aud_idx))
        z_aud_valid = F.normalize(z_aud[valid_aud_idx[:aud_subset_size]], p=2, dim=1)
        sim_aud_aud = torch.mm(z_aud_valid, z_aud_valid.t())
        off_diag_mask_aud = ~torch.eye(aud_subset_size, dtype=torch.bool)
        off_diag_sim_aud = sim_aud_aud[off_diag_mask_aud]
        valid_aud_sim_mean = off_diag_sim_aud.mean().item()
        valid_aud_sim_std = off_diag_sim_aud.std().item()
        valid_aud_sim_min = off_diag_sim_aud.min().item()
        valid_aud_sim_max = off_diag_sim_aud.max().item()
        print(f"  z_aud (VGGish)- Mean Off-Diag Sim: {valid_aud_sim_mean:.4f} | Std: {valid_aud_sim_std:.4f} | Min/Max: {valid_aud_sim_min:.4f}/{valid_aud_sim_max:.4f}")
    else:
    ...
    # 6. GENERATE VISUALS
    print("Generating and saving visuals to 'features/analysis_plots_train_features/'...")
    ...
    ax.set_title("Embedding L2 Norm Distributions (Train Features)", fontsize=14, pad=15, color='#FFFFFF', weight='bold')
    ...
    try:
        ...
        plt.suptitle("2D PCA Projections of Embedding Spaces (Train Features)", fontsize=16, color='#FFFFFF', weight='bold', y=0.98)
        plt.tight_layout()
    ...
    report_content = f"""# Multimodal Feature Analysis Report (`train_features.pt`)
...
| :--- | :--- | :--- | :--- | :--- |
| `z_img` | Student visual frame embedding | `{list(z_img.shape)}` | `{data['z_img'].dtype}` | Extracted from the **middle frame** of each video using **CLIP (ViT-B/32)**. |
| `z_aud` | Student audio embedding | `{list(z_aud.shape)}` | `{data['z_aud'].dtype}` | Extracted using **VGGish**, averaged over the video's audio timeline. |
| `v_teacher` | Teacher video embedding | `{list(v_teacher.shape)}` | `{data['v_teacher'].dtype}` | Extracted from the **entire video clip** using the vision modality of **ImageBind (huge)**. |
| `video_ids` | Original MSR-VTT video IDs | List of `{len(video_ids)}` strings | `str` | Contains IDs mapping to the MSR-VTT train video list (e.g., `'video0'`, `'video1'`). |
...
* **`z_img` (CLIP Student)**: Loaded in `{data['z_img'].dtype}` precision, with a size of {z_img.shape[1]} dimensions per video.
* **`z_aud` (VGGish Student)**: Loaded in `{data['z_aud'].dtype}`, with {z_aud.shape[1]} dimensions. Zero vectors count is **{anomalies['z_aud']['zeros']}/{num_samples} ({anomalies['z_aud']['zeros']/num_samples*100:.2f}%)**.
* **`v_teacher` (ImageBind Teacher)**: Loaded in `{data['v_teacher'].dtype}`, with {v_teacher.shape[1]} dimensions per video.

### B. Norm & Scale Distributions
* **`z_img` L2 norms**: ${norm_img.mean().item():.2f} \\pm {norm_img.std().item():.2f}$. CLIP embeddings are unnormalized raw projection outputs clustered in a clean range.
* **`v_teacher` L2 norms**: ${norm_teacher.mean().item():.2f} \\pm {norm_teacher.std().item():.2f}$. ImageBind embeddings are close to unit scale.
* **`z_aud` L2 norms**: ${norm_aud.mean().item():.2f} \\pm {norm_aud.std().item():.2f}$ (including silent ones). 
"""
    if len(non_zero_aud) > 0:
        report_content += f"* **`z_aud` L2 norms (non-zero)**: ${non_zero_aud_mean:.2f} \\pm {non_zero_aud_std:.2f}$.\n"
        
    report_content += f"""
### C. Embedding Diversity (Within-Modality Self-Similarity)
We calculated the pairwise cosine similarity between the first {subset_size_sim} samples in the dataset to evaluate representation collapse:
* **`z_img` (CLIP)**: Mean off-diagonal similarity is **${off_diag_sim_img.mean().item():.4f} \\pm {off_diag_sim_img.std().item():.4f}$**. This indicates a healthy, diverse vision space.
* **`v_teacher` (ImageBind)**: Mean off-diagonal similarity is **${off_diag_sim_teacher.mean().item():.4f} \\pm {off_diag_sim_teacher.std().item():.4f}$**. This shows clean, spread-out video embeddings.
"""

    if len(valid_aud_idx) > 1:
        report_content += f"* **`z_aud` (VGGish)**: Mean off-diagonal similarity is **${valid_aud_sim_mean:.4f} \\pm {valid_aud_sim_std:.4f}$** (computed over non-silent audio embeddings). This shows good variety in the audio space.\n"
    else:
        report_content += "* **`z_aud` (VGGish)**: Skipping pairwise similarity check because all (or almost all) audio features are zero vectors.\n"
...
1. **L2 Norms**: Visualizes the distinct norm regimes of each model. CLIP outputs are larger (${norm_img.mean().item():.1f}$), whereas ImageBind is strictly bounded below $1.0$ (${norm_teacher.mean().item():.2f}$).
...
"""
    ...
    if len(valid_aud_idx) > 1:
        report_content += "4. **Audio Features**: VGGish embeddings form a well-distributed cloud of non-zero features, unlike the collapsed zero-point seen in the buggy test feature set.\n"
    else:
        report_content += "4. **Audio Features**: The VGGish audio space is collapsed to a single point at $(0, 0)$ because the audio features are all zero vectors.\n"
...
* **Partial Progress**: The file contains **""" + str(num_samples) + """** samples out of the original 7010 videos (due to the 95% disconnected save), which is a complete and high-quality subset for training.
"""
    
    report_file_path = "features/analysis_train_features.md"
    with open(report_file_path, "w", encoding="utf-8") as f:
        f.write(report_content)
    print(f"Report written to: {os.path.abspath(report_file_path)}")
