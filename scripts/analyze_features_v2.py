"""
Analysis script for V2 features (dataV2/train_features_v2.pt, test_features_v2.pt).
...
PCA_SUBSET = 2000    # samples to use for PCA (keeps it fast)
SIM_SUBSET = 1000    # samples for pairwise self-similarity matrix
...
def pca_2d(tensor, n=PCA_SUBSET):
    """Returns (proj [n,2], explained_variance_ratio [2])."""
    x = tensor[:n].clone()
...
def pca_scree(tensor, k=20, n=PCA_SUBSET):
    """Returns explained variance ratio for top-k PCs."""
    x = tensor[:n].clone()
...
def cross_cosine_per_sample(a, b):
    """Cosine similarity between matching rows (element-wise)."""
...
def plot_per_frame_norms(z_img, split, out):
    """z_img: [N, 3, 512] — compare norm distributions across 3 frame positions."""
...
def plot_inter_frame_cosine(z_img, split, out):
    """
    For each video, compute pairwise cosine sims between its 3 frames:
...
def plot_cross_modal_analysis(z_img, z_aud, v_teacher, audio_mask, split, out):
    ...
    CLIP (512d) and ImageBind (1024d) live in different spaces so direct cosine
    similarity is undefined.  Instead we examine:
      1. Pearson correlation between per-sample L2 norms
      2. Scatter: z_img_mean norm vs v_teacher norm
      3. Pairwise-similarity structure correlation (rank corr of off-diagonal cosines)
    """
    z_img_mean = z_img.mean(dim=1)  # [N, 512]
    norm_img = torch.norm(z_img_mean, p=2, dim=1).numpy()
    norm_t   = torch.norm(v_teacher, p=2, dim=1).numpy()

    # Pearson r between norms
    r_img_t = float(np.corrcoef(norm_img, norm_t)[0, 1])

    # Pairwise similarity structure correlation (first 500 samples)
    ...
    # --- scatter: norms ---
    axes[0].scatter(norm_img, norm_t, s=6, alpha=0.35, color=COLORS["z_img"],
                    edgecolors="none")
    ...
    # --- scatter: pairwise similarity structure ---
    ...
    # --- audio norm vs teacher norm (if available) ---
    if audio_mask.sum() > 10:
        ...
        axes[2].scatter(norm_aud, norm_t_a, s=6, alpha=0.35, color=COLORS["z_aud"],
                        edgecolors="none")
...
def plot_pca(data_dict, audio_mask, split, out):
    """2D PCA for z_img_mean, z_aud (audio-only), v_teacher + scree plots."""
    z_img_mean = data_dict["z_img"].mean(dim=1)   # [N, 512]
...
def plot_train_test_comparison(train_d, test_d, train_mask, test_mask, out):
    """
    Overlay train vs test distributions for z_img mean-pool norm, teacher norm,
    and cross-modal cosine (img→teacher).
    """
    ...
    def inter_frame_cosine(d):
        # early <-> late frame cosine sim per video (summary of temporal diversity)
...
# ---------------------------------------------------------------------------
# main analysis for one split
# ---------------------------------------------------------------------------

def analyze_split(d, split, out):
    banner(f"ANALYZING {split.upper()}")
    z_img     = d["z_img"]      # [N, 3, 512]
    z_aud     = d["z_aud"]      # [N, 128]
    v_teacher = d["v_teacher"]  # [N, 1024]
    has_audio = d["has_audio"]  # [N]
    ...
    # cross-modal: norm correlation (direct cosine undefined — different dims)
    aud_mask = has_audio
    ...
    print("  (direct cosine undefined: CLIP=512d vs IB=1024d)")

    # pairwise self-similarity (subset)
...
def main():
    ...
    banner("V2 FEATURE ANALYSIS")

    train_path = os.path.join(data_dir, "train_features_v2.pt")
    test_path  = os.path.join(data_dir, "test_features_v2.pt")
